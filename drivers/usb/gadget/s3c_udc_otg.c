...
void usb_lock_dvfs(bool enable)
{
	if (dvfs_locked != enable) {
		dvfs_locked = enable;
		if (enable)
			s5pv210_lock_dvfs_high_level(DVFS_LOCK_TOKEN_8, L2);
		else
			s5pv210_unlock_dvfs_high_level(DVFS_LOCK_TOKEN_8);
	}
}
EXPORT_SYMBOL(usb_lock_dvfs);
