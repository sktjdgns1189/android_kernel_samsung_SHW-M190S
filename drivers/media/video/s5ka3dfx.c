...
static struct s5ka3dfx_regset_table init_qvga_reg[] = {
	S5KA3DFX_REGSET_TABLE_ELEMENT(0, s5ka3dfx_init_qvga_reg),
};
static struct s5ka3dfx_regset_table init_qvga_vt_reg[] = {
	S5KA3DFX_REGSET_TABLE_ELEMENT(0, s5ka3dfx_init_qvga_vt_reg),
};
				   ...
				   struct v4l2_control *ctrl)
{
	...
	/* S1-KOR HD VT : requested by SKT */
	if ((state->vt_mode == VT_MODE_S) || (state->vt_mode == VT_MODE_NS)) {
		ev = brightness_vt;
...
static int s5ka3dfx_init(struct v4l2_subdev *sd, u32 val)
{
	...
	if (state->vt_mode == VT_MODE_OFF) {
		...
		if (state->framesize_index == S5KA3DFX_PREVIEW_QVGA)
			err = s5ka3dfx_write_regset_table(sd, init_qvga_vt_reg);
		else
