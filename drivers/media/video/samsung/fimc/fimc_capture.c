...
//HD VT : requested by SKT - For normal display rotated preview image of mega camera
static int vtmode = 0;		// 0: vt off,  1: skt hd vt mode,  2: normal vt mode
...
static int fimc_camera_start(struct fimc_control *ctrl)
{
	...
	if (ret < 0) {
...
#if 1	//HD VT : requested by SKT - For normal display rotated preview image of mega camera

		//printk(KERN_ERR "%s : device_id string %s \n", __func__, ctrl->cam->sd->name);		//ktj_test
		//printk(KERN_ERR "%s : fmt_w %d, fmt_h %d \n", __func__, ctrl->cap->fmt.width, ctrl->cap->fmt.height);		//ktj_test
		//printk(KERN_ERR "%s : discrete_w %d, discrete_h %d \n", __func__, cam_frmsize.discrete.width, cam_frmsize.discrete.height, ctrl->cap->rotate, ctrl->id);		//ktj_test
		//printk(KERN_ERR "%s : vtmode %d, device_id %d rotate %d ctrl->id : %d \n", __func__, vtmode, device_id);		//ktj_test

		if (device_id == 0 && vtmode == 1 && (ctrl->cap->rotate == 90 || ctrl->cap->rotate == 270)) {
			if((cam_frmsize.discrete.width > cam_frmsize.discrete.height)
			&& (cam_frmsize.discrete.width >= ((cam_frmsize.discrete.height*4)/3))
			) {
				//printk(KERN_ERR "In case of (width > height)&(width >= height*4/3)\n");
				ctrl->cam->window.left = ALIGN(((cam_frmsize.discrete.width - ((cam_frmsize.discrete.height*3)/4))/2), 16);
				ctrl->cam->window.top = 0;
				ctrl->cam->window.width = ALIGN(((cam_frmsize.discrete.height*3)/4), 16);
				ctrl->cam->window.height = cam_frmsize.discrete.height;
				//printk(KERN_ERR "%s : Align Result - left %d, width %d\n", __func__, ctrl->cam->window.left, ctrl->cam->window.width);
			} else {
				//printk(KERN_ERR "Forced setting !!!\n");
			...
			//printk(KERN_ERR "vtmode = 1, device = %d, cam->width = %d, cam->height = %d\n", device_id, ctrl->cam->width, ctrl->cam->height);
		} else if (device_id != 0 && vtmode == 0) {		//S1_EUR
			ctrl->cam->window.left = 136;
			ctrl->cam->window.top = 0;
			ctrl->cam->window.width = 368;
			ctrl->cam->window.height = 480;
		...
		} else if (device_id != 0 && vtmode != 1) {		//S1_EUR
			ctrl->cam->window.left = 136;
			ctrl->cam->window.top = 0;
			ctrl->cam->window.width = 368;
			ctrl->cam->window.height = 480;
...
int fimc_streamon_capture(void *fh)
{
	...
	if(ret < 0){
		...
		//printk(KERN_ERR "%s : device_id string %s \n", __func__, ctrl->cam->sd->name);		//ktj_test
		//printk(KERN_ERR "%s : fmt_w %d, fmt_h %d \n", __func__, ctrl->cap->fmt.width, ctrl->cap->fmt.height);		//ktj_test
		//printk(KERN_ERR "%s : discrete_w %d, discrete_h %d \n", __func__, cam_frmsize.discrete.width, cam_frmsize.discrete.height);		//ktj_test
		//printk(KERN_ERR "%s : vtmode %d, device_id %d \n", __func__, vtmode, device_id);		//ktj_test
		...
		if (device_id == 0 && vtmode == 1 && (ctrl->cap->rotate == 90 || ctrl->cap->rotate == 270)) {
			...
			ctrl->cam->width = cam_frmsize.discrete.width;
			ctrl->cam->height = cam_frmsize.discrete.height;
			//printk(KERN_ERR "vtmode = 1, device = 0, cam->width = %d, cam->height = %d\n", ctrl->cam->width, ctrl->cam->height);
		} else if (device_id != 0 && vtmode == 0) {		//S1_EUR
			ctrl->cam->window.left = 136;
			ctrl->cam->window.top = 0;
			ctrl->cam->window.width = 368;
			...
			fimc_err("[KTJ]cam_frmsize.discrete.width x cam_frmsize.discrete.height : %d x %d", cam_frmsize.discrete.width, cam_frmsize.discrete.height);
		}
	}
#else
	ret = subdev_call(ctrl, video, enum_framesizes, &cam_frmsize);
	...
	if (cap->fmt.pixelformat != V4L2_PIX_FMT_JPEG) {
		...
		if ((device_id != 0) && (vtmode == 0)) {		//S1_EUR
			ctrl->cap->rotate = 90;
