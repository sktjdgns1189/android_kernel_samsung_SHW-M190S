...
static struct delayed_work g_g2d_domain_work;
...
#else
			schedule_delayed_work(&g_g2d_domain_work, msecs_to_jiffies(1000));
#endif
		}
	}
}
...
static void sec_g2d_domain_work(struct work_struct *work)
{
	sec_g2d_clk_disable();
}
...
static int sec_g2d_probe(struct platform_device *pdev)
{
	...
	pr_info("g2d: start probe : name=%s num=%d res[0].start=0x%x res[1].start=0x%x\n",
	...
	g_g2d_clk = clk_get(&pdev->dev, "fimg2d");
	if (g_g2d_clk == NULL) {
...
#else
	INIT_DELAYED_WORK(&g_g2d_domain_work, sec_g2d_domain_work);
	schedule_delayed_work(&g_g2d_domain_work, msecs_to_jiffies(1000));
#endif

	pr_info("g2d: sec_g2d_probe ok!\n");
...
#else
	cancel_delayed_work_sync(&g_g2d_domain_work);
#endif
