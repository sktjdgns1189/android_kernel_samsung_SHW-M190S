...
static bool write_ainfo(void)
{
	...
	if (ret < 0)
		printk(KERN_ERR "Can't write ainfo data through i2c bus\n");
...
static bool write_an(void)
{
	...
	if (ret < 0)
		printk(KERN_ERR "Can't write an data through i2c bus\n");
...
static bool write_aksv(void)
{
	...
	if (ret < 0)
		printk(KERN_ERR "Can't write aksv data through i2c bus\n");
...
static bool read_bcaps(void)
{
	...
	printk(KERN_DEBUG "[HDMI] %s ++\n", __func__);

	ret = ddc_read(HDCP_Bcaps, bcaps, BCAPS_SIZE);

	if (ret < 0) {
		printk(KERN_ERR "[HDCP] %s : Can't read bcaps data from i2c bus\n", __func__);
		return false;
	}
...
static bool read_again_bksv(void)
{
	...
	if (ret < 0) {
		printk(KERN_ERR "[HDCP] %s : Can't read bk_sv data from i2c bus\n", __func__);
		return false;
	}
	...
	if ((no_zero == 20) && (no_one == 20)) {
		HDCPPRINTK("Sucess: no_zero, and no_one is 20\n");
		...
		*/
		...
		printk(KERN_ERR "[HDCP] %s : Failed: no_zero or no_one is NOT 20\n", __func__);
		return false;
	}
}
...
static bool compare_r_val(void)
{
	...
	for (i = 0; i < R_VAL_RETRY_CNT; i++) {
		...
		if (ret < 0) {
			printk(KERN_ERR "[HDCP] %s : Can't read r data from i2c bus\n", __func__);
			return false;
		}

#ifdef S5P_HDCP_AUTH_DEBUG
		AUTHPRINTK("retries :: %d\n", i);
		printk(KERN_DEBUG "\t\t\t Rx(ddc) ->");
		printk(KERN_DEBUG "rj[0]: 0x%02x, rj[1]: 0x%02x\n",
						rj[0], rj[1]);
		printk(KERN_DEBUG "\t\t\t Tx(register) ->");
		printk(KERN_DEBUG "ri[0]: 0x%02x, ri[1]: 0x%02x\n",
						ri[0], ri[1]);
#endif
		...
		if ((ri[0] == rj[0]) && (ri[1] == rj[1]) && (ri[0] | ri[1])) {
			...
			writel(Ri_MATCH_RESULT__NO,
				hdmi_base + S5P_HDCP_CHECK_RESULT);
			printk(KERN_ERR "[HDCP] %s : R0, R0' is not matched!!\n", __func__);
			printk(KERN_DEBUG "retries :: %d\n", i);
			printk(KERN_DEBUG "\t\t\t Rx(ddc) ->");
			printk(KERN_DEBUG "rj[0]: 0x%02x, rj[1]: 0x%02x\n",
				rj[0], rj[1]);
			printk(KERN_DEBUG "\t\t\t Tx(register) ->");
...
void reset_authentication(void)
{
	...
	printk(KERN_ERR "[HDCP] reset_authentication() +\n");
...
static int hdcp_loadkey(void)
{
	...
	if (readb(hdmi_base + S5P_EFUSE_STATUS) & EFUSE_ECC_FAIL) {
		printk(KERN_DEBUG "[HDCP] %s : Can't load key from fuse ctrl.\n", __func__);
		return -EINVAL;
	}
	...
	if (readl(hdmi_base + S5P_HDCP_CHECK_RESULT) ==
		...
		while (time_out) {
			...
			if (readl(hdmi_base + S5P_STATUS) & AUTHENTICATED) {
				...
				printk(KERN_DEBUG "[HDCP] %s : Encryption start!!\n", __func__);
		...
		s5p_hdmi_mute_en(true);
		printk(KERN_DEBUG "[HDCP] %s : Encryption stop!!\n", __func__);
	}
}
...
static int check_repeater(void)
{
	...
	while (j <= 50) {
		...
		if (ret < 0) {
			printk(KERN_DEBUG "[HDCP] %s : Can't read bcaps data from i2c bus\n", __func__);
			return false;
		}
	...
	if (ret < 0) {
		printk(KERN_DEBUG "[HDCP] %s : Can't read status data from i2c bus\n", __func__);
		return false;
	}
	...
	if (dev_cnt) {
		...
		if (ret < 0) {
			printk(KERN_DEBUG "[HDCP] %s : Can't read ksv fifo!!\n", __func__);
			return false;
		}
	...
	if (ret < 0) {
		printk(KERN_DEBUG "[HDCP] %s : Can't read sha_1_hash data from i2c bus\n", __func__);
		return false;
	}
	...
	printk(KERN_INFO "[HDCP] check_repeater() : auth status %d\n", stat);
...
static bool try_read_receiver(void)
{
	...
	HDCPPRINTK("try_read_receiver() +\n");
	printk(KERN_ERR "[HDCP] try_read_receiver() +\n");
	...
	for (i = 0; i < 400; i++) {
		...
		if (hdcp_info.auth_status != RECEIVER_READ_READY) {
			...
			printk(KERN_ERR "hdcp stat. changed!!"
				"failed attempt no = %d\n\r", i);
		...
		if (ret) {
			...
			printk(KERN_ERR "can't read bcaps!!"
				"failed attempt no=%d\n\r", i);
	}
...
bool __s5p_stop_hdcp(void)
{
	...
	printk(KERN_DEBUG "[HDCP] %s : HDCP ftn. Stop!!\n", __func__);
	...
	*/
	...
	/* disable encryption */
	printk(KERN_DEBUG "[HDCP] %s : Stop Encryption by Stop!!\n", __func__);
	writel(HDCP_ENC_DISABLE, hdmi_base + S5P_ENC_EN);
	s5p_hdmi_mute_en(true);
...
bool __s5p_start_hdcp(void)
{
	...
	printk(KERN_DEBUG "[HDCP] __s5p_start_hdcp() +\n");
	...
	sw_reset = false;
	printk(KERN_DEBUG "[HDCP] __s5p_start_hdcp() : Stop Encryption by Start!!\n");
...
static void bksv_start_bh(void)
{
	...
	if (!ret) {
		printk(KERN_DEBUG "[HDCP] %s :  read_bcaps failed\n", __func__);
		ret = try_read_receiver();

		if (!ret) {
			printk(KERN_DEBUG "Can't read bcaps!! retry failed!!\n"
	...
	if (!ret) {
		printk(KERN_DEBUG "Can't read bksv!!"
...
static void second_auth_start_bh(void)
{
	...
	if (!ret) {
		printk(KERN_INFO "[HDCP] HDCP_EVENT_SECOND_AUTH_START : read_bcaps failed \n");
		ret = try_read_receiver();
