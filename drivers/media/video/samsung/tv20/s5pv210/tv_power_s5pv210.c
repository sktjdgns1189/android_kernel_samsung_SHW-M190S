...
{
	printk(KERN_INFO "[TVPM]__s5p_tv_poweron() : 0x%08x\n", readl(S3C_VA_SYS + 0xE804));

	writel(readl(S3C_VA_SYS + 0xE804) | 0x1, S3C_VA_SYS + 0xE804);

#if 0	/* controlled by tv_regulator in s5p_tv_clk_gate() */
...
{
	printk(KERN_INFO "[TVPM] __s5p_tv_poweroff()\n");

	__s5p_tv_powerset_dac_onoff(0);

	writel(readl(S3C_VA_SYS + 0xE804) & ~(0x1), S3C_VA_SYS + 0xE804);
	
#if 1	/* controlled by tv_regulator in s5p_tv_clk_gate() */
	writel(readl(S5P_NORMAL_CFG) & ~TVPWR_SUBSYSTEM_ACTIVE, S5P_NORMAL_CFG);
