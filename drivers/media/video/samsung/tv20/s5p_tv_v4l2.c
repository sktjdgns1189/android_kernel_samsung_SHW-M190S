	...
	struct v4l2_fmtdesc *f)
{
	...
	if (index >= S5P_TVOUT_MAX_O_FMT_DESC) {
		V4L2ERR("exceeded S5P_TVOUT_MAX_O_FMT_DESC\n");
		return -EINVAL;
	}
	...
	void *fh, struct v4l2_fmtdesc *f)
{
	...
	if (index >= S5P_TVOUT_MAX_O_OVERLAY_FMT_DESC) {
		V4L2ERR("exceeded S5P_TVOUT_MAX_O_OVERLAY_FMT_DESC\n");
		return -EINVAL;
	}
	...
	case V4L2_BUF_TYPE_VIDEO_OUTPUT: {
		...
		if (vparam.pix_fmt.field == V4L2_FIELD_NONE) {
			...
			case V4L2_PIX_FMT_NV12T:
				...
				break;
			default:
				V4L2ERR("src img format not supported\n");
				break;
			}
			...
			V4L2ERR("this field id not supported\n");
		}
		break;
	}
...
static int s5p_tv_v4l2_overlay(struct file *file, void *fh, unsigned int i)
{
	...
	if (s5ptv_status.suspend_status == true || !(s5ptv_status.hpd_status)) {
		...
		V4L2ERR("suspend mode/hdmi cable is not inserted\n");
		return 0;
	} else {
		if (start)
...
static int s5p_tv_v4l2_s_std(struct file *file, void *fh, v4l2_std_id *norm)
{
	...
	if (i >= S5P_TVOUT_MAX_STANDARDS || s5ptv_status.v4l2.std == NULL) {
		V4L2ERR("(ERR) There is no tv-out standards :\
			index = 0x%08Lx\n", std_id);
		return -EINVAL;
	}
...
#endif
	default:
		V4L2ERR("(ERR) not supported standard id :\
			index = 0x%08Lx\n", std_id);
		return -EINVAL;
		break;
	}
	...
	struct v4l2_output *a)
{
	...
	if (index >= S5P_TVOUT_MAX_O_TYPES) {
		V4L2ERR("exceeded supported output!!\n");
		return -EINVAL;
	}
...
long s5p_tv_v_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case VIDIOC_ENUMSTD: {
		...
		if (p->index >= S5P_TVOUT_MAX_STANDARDS) {
			V4L2ERR("exceeded S5P_TVOUT_MAX_STANDARDS (%d)\n", 
				p->index);
			return -EINVAL;
		}
