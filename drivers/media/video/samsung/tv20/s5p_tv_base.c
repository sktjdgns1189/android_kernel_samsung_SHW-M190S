...
{
	if (on) {
		printk(KERN_DEBUG "tv_phy_power (on)\n");
		__s5p_tv_poweron();
		/* on */
		 ...
		 */
		printk(KERN_DEBUG "tv_phy_power (off)\n");
...
{
	printk(KERN_DEBUG "s5p_tv_clk_gate ++ (%d)\n", on);
	if (on) {
	...
	printk(KERN_DEBUG "s5p_tv_clk_gate --\n");
...
{
	int ret = 0;
	printk(KERN_DEBUG "[TVOUT] %s \n", __func__);
	TVout_LDO_ctrl(true);
	Isdrv_open = 1;
	mutex_lock(mutex_for_fo);

	if (s5ptv_status.tvout_output_enable) {
		printk(KERN_ERR "tvout drv. already used !!\n");
		ret =  -EBUSY;
		goto drv_used;
	}
...
{
	printk(KERN_DEBUG "[TVOUT] %s ++: vp_layer_enable(%d),"
		" tvout_output_enable(%d)\n", __func__,
 		s5ptv_status.vp_layer_enable,
		s5ptv_status.tvout_output_enable);
...
static int vo_open(int layer, struct file *file)
{
	...
	if (!s5ptv_status.tvout_output_enable) {
		printk("check tvout start !!\n");
		ret =  -EACCES;
		goto resource_busy;
	}

	if (s5ptv_status.grp_layer_enable[layer]) {
		printk("grp %d layer is busy!!\n", layer);
		ret =  -EBUSY;
		goto resource_busy;
	}
...
void TVout_LDO_ctrl(int enable)
{
	...
	printk(KERN_DEBUG "[TVOUT] %s ++", __func__);
	...
	if (enable == true) {
		if (!IsPower_on) {
			...
			printk(KERN_DEBUG "%s: LDO3_8 is enabled by TV\n", __func__);
			IsPower_on = 1;
			msleep(120);
		}
	}
...
#if 0
		if(gpio_get_value(GPIO_DOCK_INT)) {
			if(s5ptv_status.suspend_status)
				msleep(520);
			...
			if((gpio_get_value(GPIO_DOCK_INT))&&(IsPower_on)&& (!Isdrv_open)) {
		...
		if (IsPower_on && (!Isdrv_open)) {
			...
			regulator_disable(s5ptv_status.tv_osc);
			IsPower_on = 0;
			printk(KERN_DEBUG "%s: LDO3_8 is disabled by TV\n", __func__);
		}
#endif
...
void s5p_handle_cable(void)
{
	...
	if(previous_hpd_status == s5ptv_status.hpd_status) {
		printk(KERN_DEBUG "[TVOUT] same hpd_status value: %d\n", previous_hpd_status);
		return;
	}

	if(s5ptv_status.hpd_status) {
		printk(KERN_DEBUG "[TVOUT] hdmi cable is connected\n");
#ifdef SEC_HOPPIN_TV
		if(Isdrv_open && s5ptv_status.hdcp_en) {			
		...
		if(s5ptv_status.suspend_status) {
			printk(KERN_DEBUG "[TVOUT] s5ptv_status.suspend_status is high \n");
			return;
		}

#ifndef SEC_HOPPIN_TV
		s5p_tv_clk_gate( true );
		//if(s5ptv_status.tvout_param.out_mode == TVOUT_OUTPUT_HDMI ||
		//   s5ptv_status.tvout_param.out_mode == TVOUT_OUTPUT_HDMI_RGB)
		//	tv_phy_power( true );
...
static int __devinit s5p_tv_probe(struct platform_device *pdev)
{
...
#ifdef CONFIG_CPU_S5PV210
#ifdef CONFIG_HDMI_HPD
	if(0 == gpio_get_value(GPIO_DOCK_INT))  //docking station attached
		s5ptv_status.hpd_status= s5p_hpd_get_state();
	else 
	...
	/* Interrupt */
	TVOUT_IRQ_INIT(irq_num, ret, pdev, 0, out,
			__s5p_mixer_irq, "mixer");
...
#ifdef CONFIG_CPU_S5PC100	
	TVOUT_IRQ_INIT(irq_num, ret, pdev, 3, out_hpd_irq, 
			__s5p_hpd_irq, "hpd");
...
{
	printk(KERN_DEBUG "[TVBASE] %s: (hpd_status = %d)++\n",
		__func__, s5ptv_status.hpd_status);

	mutex_lock(mutex_for_fo);
	s5ptv_status.suspend_status = true;

	if (!(s5ptv_status.hpd_status)) {
		printk(KERN_DEBUG "[TVBASE] %s --\n", __func__);
		mutex_unlock(mutex_for_fo);
		return;
	} else  {
#ifdef SEC_HOPPIN_TV	
		if (!Isdrv_open)	{
			printk(KERN_DEBUG "[TVBASE] %s: TV drv not opened!\n", __func__);
			mutex_unlock(mutex_for_fo);
			return;
		}
#endif
		...
		if (s5ptv_status.grp_layer_enable[1] ) {
			_s5p_grp_stop(VM_GPR1_LAYER);
			s5ptv_status.grp_layer_enable[VM_GPR1_LAYER] = true;
		}
