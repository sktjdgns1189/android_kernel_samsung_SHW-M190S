...
#if defined(CONFIG_M115S)
extern void lcd_cfg_gpio_drvstrength(int pclk, int data);
#endif
...
void mDNIe_Set_Mode(Lcd_mDNIe_UI mode, u8 mDNIe_Outdoor_OnOff)
{
	...
	{
		printk(KERN_INFO"override lcd drvstrength");
		lcd_cfg_gpio_drvstrength (4, 1);
	}
	else
	{
		printk(KERN_INFO"restore drvstrength");
		lcd_cfg_gpio_drvstrength (4, 2);
	}
#endif
	...
	for(i=0; i<count; i++) 
    {
		...
		if(ret == 1)
		{
			*(strchr(mode_name,'[')) = 0;
			...
			else if(strcmp(mode_name,"mDNIe_DMB")==0)
			{
				mode = mDNIe_DMB;
			}
			else if(strcmp(mode_name,"mDNIe_DMB_Warm")==0)
			{
				mode = mDNIe_DMB_Warm;
			}
			else if(strcmp(mode_name,"mDNIe_DMB_WO_Mode")==0)
			{
				mode = mDNIe_DMB_WO_Mode;
			}
			else if(strcmp(mode_name,"mDNIe_DMB_Cold")==0)
			{
				mode = mDNIe_DMB_Cold;
			}
			else if(strcmp(mode_name,"mDNIe_DMB_CO_Mode")==0)
			{
				mode = mDNIe_DMB_CO_Mode;
			}
			else if(strcmp(mode_name,"mDNIe_DMB_Outdoor_Mode")==0)
			{
				mode = mDNIe_DMB_Outdoor_Mode;
			}
