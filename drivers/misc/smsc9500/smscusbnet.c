...
struct device *smsc9500_dev = NULL;
int ethernet_state = 0;
struct device *ether_dev = NULL;
//int rx_skb_count = 0;
//static struct wake_lock smscusbnet_wake_lock;
extern void set_ethernet_state(int state);
extern bool charging_mode_get(void);

//DEFINE_MUTEX(state_lock);
//DEFINE_MUTEX(function_lock);

int get_smsc_ethernet_state(void)
{
	int ret = 0;
...
inline int send_uevent_smsc_state(struct kobject *kobj, int data)
{
	//char inf_buf[60];
	char state_buf[60];
	//char *envp[3];
	...
	if(!kobj) {
		printk("[send_uevent_smsc_state-error] ether_dev not ready!!\n");
		return -1;
	}
	...
	if(data==0)
		sprintf(state_buf, "ETHERNET_STATE=notready"); //disconnect or probe fail
	else if(data==1)
		sprintf(state_buf, "ETHERNET_STATE=unplugged"); // cable link down
	else if(data==2)
		sprintf(state_buf, "ETHERNET_STATE=plugged"); // cable link up
	else
		sprintf(state_buf, "ETHERNET_STATE=unknown"); // unknown

	//sprintf(inf_buf, "INTERFACE=eth1");

	//devpath = kobject_get_path(kobj, GFP_KERNEL);
	//subsystem = kobject_name(kobj);

    //printk("[smsc9500-uevent] %s %s\n", devpath, subsystem);

	envp[envp_offset++] = state_buf;
	//envp[envp_offset++] = inf_buf;
	envp[envp_offset] = NULL;
	printk("[smscusbnet] send uevent state %d\n",data);
	//kobject_uevent_env(&ether_dev->kobj, KOBJ_CHANGE, state_buf);
	kobject_uevent_env(kobj, KOBJ_CHANGE, envp);

	//or
	//kobject_uevent(&ether_dev->kobj, KOBJ_CHANGE);
...
static inline int send_uevent_smsc_addr(struct kobject *kobj, struct net_device *net)
{
	char mac_buf[60];
	char *envp[2];
	int envp_offset = 0;
	...
	if(!kobj) {
		printk("[send_uevent_smsc_addr-error] ether_dev not ready!!\n");
		return -1;
	}
	...
	/*
	printk("[send_uevent_smsc_addr] address = %02x:%02x:%02x:%02x:%02x:%02x\n",
			net->dev_addr [0], net->dev_addr [1],
			net->dev_addr [2], net->dev_addr [3],
			net->dev_addr [4], net->dev_addr [5]);
	*/

	sprintf(mac_buf, "ETHERNET_ADDR=%02x:%02x:%02x:%02x:%02x:%02x",
		    net->dev_addr [0], net->dev_addr [1],
			net->dev_addr [2], net->dev_addr [3],
			net->dev_addr [4], net->dev_addr [5]); //MAC address
	...
	envp[envp_offset++] = mac_buf;
	envp[envp_offset] = NULL;
	printk("[smscusbnet] send uevent address\n");
	//kobject_uevent_env(&ether_dev->kobj, KOBJ_CHANGE, state_buf);
	kobject_uevent_env(kobj, KOBJ_CHANGE, envp);
...
int smscusbnet_change_mtu (struct net_device *net, int new_mtu)
{
	...
	//printk("\n\n[smscusbnet] smscusbnet_change_mtu in --------->\n");
	...
	//printk("\n\n[smscusbnet] smscusbnet_change_mtu out <---------\n");
...
{
	//printk("[smscusbnet] smscusbnet_defer_kevent : (%d)\n", work);
...
{
	//printk("[smscusbnet] smscusbnet_defer_myevent : (%d)\n", work);
	...
	if(ethernet_state) {
		...
		printk("skkp smscusbnet_defer_myevent, ethernet_state is 0\n");
	}
}
...
void smscusbnet_linkpolling(unsigned long ptr)
{
	...
	if (dev==NULL) return;	
	//devdbg(dev,"[smscnet] smscusbnet_linkpolling, EVENT_LINK_RESET\n");
	smscusbnet_defer_myevent(dev, EVENT_LINK_RESET);

	//printk("[smscnet] smscusbnet_linkpolling...\n");

#if (LINUX_VERSION_CODE > KERNEL_VERSION(2,6,18)) && defined(CONFIG_PM)
		if(dev->dynamicSuspend){
		   //devdbg(dev,"[smscnet] smscusbnet_linkpolling, EVENT_IDLE_CHECK\n");
		...
		if(dev->netDetach){
			...
			if(!netif_carrier_ok(dev->net) && dev->linkcheck++ > 2){//Check 2 times in case of conflict with resume
			    //devdbg(dev,"[smscnet] smscusbnet_linkpolling, EVENT_LINK_DOWN\n");
...
static void rx_submit (struct usbnet *dev, struct urb *urb, int flags)
{
	...
	//printk("rx_submit, alloc_skb\n");
	//printk("(1)rx_skb_count = %d\n", rx_skb_count);

#ifndef RX_OFFSET
	//printk("alloc_skb, size : %d, flags %d\n",size, flags);
...

{
	...
	//printk("[rx_complete] urb->actual_length = %d\n",urb->actual_length);
	entry->state = rx_done;
	entry->urb = NULL;
...
block:
	...
	//sk_buff debugging
	//printk("[rx_complete] sk_buff : rxq len = %d\n", (&dev->rxq)->qlen);
...

{
	...
	//devdbg(dev, "[smscusbnet] intr_complete...\n");
...
static void  smscusbnet_bh (unsigned long param)
{
	...
	if (dev==NULL) return;
	//devdbg(dev,"[smscusbnet] smscusbnet_bh.....\n");
	//printk("[smscusbnet] smscusbnet_bh in --------->\n");
	...
	while ((skb = skb_dequeue (&dev->done))) {
		...
		switch (entry->state) {
			...
			usb_free_urb (entry->urb);
				//printk("usb_free_urb\n");
			dev_kfree_skb (skb);
				//printk("dev_kfree_skb\n");

			if(dev->device_id == DEV_SMSC7500){
				printk("DEV_SMSC7500??--->nonononononoononononono!!!\n");
				if(!dev->txq.qlen){
				...
				if(!dev->txq.qlen) {
					//printk("smscusbnet_bundle_skb_ximt->\n");
	...
	// or are we maybe short a few urbs?
		...
		//printk("smscusbnet_bh........444\n");
		if (((operational_mode)&&(!dev->StopSummitUrb)) ||(!(operational_mode) ))  {
			int	temp = dev->rxq.qlen;
			int	qlen = RX_QLEN (dev);

			//printk("smscusbnet_bh........555... rxqlen = %d\n", qlen);

			if (temp < qlen) {
				struct urb	*urb;
				int		i;

				//printk("smscusbnet_bh, temp = %d, rxqlen = %d\n",temp,qlen);
		...
		if(!dev->tx_hold_on_completion){
			//printk("smscusbnet_bh........777\n");
			if (dev->txq.qlen < TX_QLEN (dev))
				netif_wake_queue (dev->net);
		}
	}

	//printk("[smscusbnet] smscusbnet_bh out <---------\n");
}
...
int smscusbnet_stop (struct net_device *net)
{
	...
	//devdbg(dev,"[smscusbnet] smscusbnet_stop...\n");
	//mutex_lock(&function_lock);
	printk("%s >>\n", __func__);
...
int smscusbnet_open (struct net_device *net)
{
	...
	//mutex_lock(&function_lock);
	printk("%s >>\n", __func__);
	//devdbg(dev, "[smscusbnet] smscusbnet_open in !!!!!!!!\n");

#if 0
	if(send_uevent_smsc_addr(&ether_dev->kobj, net)<0)
	{
		printk("[smscusbnet] send uevent addr error\n");
	}
#endif
	...
	devdbg (dev,"info->reset && (retval = info->reset (dev)) \n");
		if (netif_msg_ifup (dev))
			...
			info->description);
		printk("[smscusbnet] smscusbnet_open, ->reset error!!!!!\n");
		goto done;
	}
	...
	if (info->check_connect && (retval = info->check_connect (dev)) < 0) {
		if (netif_msg_ifup (dev))
			deverr (dev, "can't open; %d", retval);
		goto done;
	}
	//else if(!info->check_connect) {
		//printk("[smscusbnet] smscusbnet_open, check_connect is NULL (not error)!!!!!\n");
	//}
	...
	//printk("[smscusbnet] tasklet_schedule (&dev->bh)............\n");
	tasklet_schedule (&dev->bh);
...
static void kevent(struct work_struct *work)
{
	...
	if (test_bit (EVENT_TX_HALT, &dev->flags)) {
        ...
        set_bit (EVENT_DEV_RECOVERY, &dev->flags);
		printk("%s: EVENT_TX_HALT->RECOVERY\n", __func__);
	...
	if (test_bit (EVENT_RX_HALT, &dev->flags)) {
        ...
        //Set recovery flag.
        set_bit (EVENT_DEV_RECOVERY, &dev->flags);
		printk("%s: EVENT_RX_HALT->RECOVERY\n", __func__);
...
static void myevent(struct work_struct *work)
{
	...
	//printk("[smscusbnet] myenvent ->....\n");
#if (LINUX_VERSION_CODE > KERNEL_VERSION(2,6,18)) && defined(CONFIG_PM)
    if (test_bit (EVENT_IDLE_CHECK, &dev->flags)) {
		//devdbg(dev,"[smscusbnet-myevent] EVENT_IDLE_CHECK\n");
		...
		&& !dev->udev->autosuspend_disabled
			...
			if((dev->idleCount >= PM_IDLE_DELAY) && 
				...
				//printk("smscusbnet] go to suspendDevice \n");
				suspendDevice(dev);
			}
    ...
    if (test_bit (EVENT_LINK_DOWN, &dev->flags)) {
		//devdbg(dev,"[smscusbnet-myevent] EVENT_LINK_DOWN\n");
    ...
    if (test_bit (EVENT_IDLE_RESUME, &dev->flags)) {
		//devdbg(dev,"[smscusbnet-myevent] EVENT_IDLE_RESUME\n");
	...
	if (test_bit (EVENT_LINK_RESET, &dev->flags)) {
		//devdbg(dev,"[smscusbnet-myevent] EVENT_LINK_RESET\n");
		struct driver_info 	*info = dev->driver_info;
		int			retval = 0;

		clear_bit (EVENT_LINK_RESET, &dev->flags);

		//printk("[smscusbnet] myevent link reset\n");
        if(info->link_reset && (retval = info->link_reset(dev)) < 0) {
			...
			//devinfo(dev, "link reset success (%d) usbnet usb-%s-%s, %s",
			//	retval,
			//	dev->udev->bus->bus_name, dev->udev->devpath,
			//	info->description);
		//}
	...
	if (test_bit (EVENT_SET_MULTICAST, &dev->flags)) {
		//devdbg(dev,"[smscusbnet-myevent] EVENT_SET_MULTICAST\n");
		struct driver_info 	*info = dev->driver_info;
		int			retval = 0;
...
void smscusbnet_tx_timeout (struct net_device *net)
{
	...
	//printk("\n\n[smscusbnet] smscusbnet_tx_timeout in --------->\n");
	...
	//printk("\n\n[smscusbnet] smscusbnet_tx_timeout out <---------\n");
}
...
static int smscusbnet_bundle_skb_ximt(struct usbnet *dev, struct sk_buff_head *q)
{
	...
	if(pkt_cnt > 1){
		...
		//printk("one packet, __skb_dequeue...%d\n",SkbSize);
		skb = __skb_dequeue(q);
	}
	spin_unlock_irqrestore(&q->lock, flags);
...
int smscusbnet_start_xmit (struct sk_buff *skb, struct net_device *net)
{
	...
	//printk("[smscusbnet] smscusbnet_start_xmit...\n");
	//printk("\n\n[smscusbnet] smscusbnet_start_xmit in --------->\n");
    ...
    if(dev->uintf->pm_usage_cnt <= 0){
		...
		//devdbg(dev,"[smscnet] smscusbnet_start_xmit, EVENT_IDLE_RESUME\n");
	...
	//printk("\n\n[smscusbnet] smscusbnet_start_xmit out <---------\n");
...
void smscusbnet_disconnect (struct usb_interface *intf)
{
	...
	//mutex_lock(&state_lock);
	ethernet_state = 0; //notready
	if(send_uevent_smsc_state(&ether_dev->kobj, ethernet_state)==0)
	{
		printk("[smscusbnet_disconnect] kobject_uevent for ethernet state %d\n", ethernet_state);
	}
	//mutex_unlock(&state_lock);
	...
	/* during disconnect, stop() is called, so ethernet_state is set to '4' */
	/* so, set ethernet_state to '0' again */
	//mutex_lock(&state_lock);
	ethernet_state = 0;
	set_ethernet_state(0);
	printk("%s <<\n", __func__);
	//mutex_unlock(&state_lock);
	//wake_lock_timeout(&smscusbnet_wake_lock, 5 * HZ);
	//mutex_unlock(&function_lock);
}
...
smscusbnet_probe (struct usb_interface *udev, const struct usb_device_id *prod)
{
	...
	printk("%s, op_mode = %d\n", __func__, operational_mode);

	info = (struct driver_info *) prod->driver_info;
	if (!info) {
		printk("[smscusbnet] probe, driver_info error!\n");
		return -ENODEV;
	}
	...
	//wake_lock(&smscusbnet_wake_lock);

	// set up our own records
	net = alloc_etherdev(sizeof(*dev));
	if (!net) {
		deverr (dev,"can't kmalloc dev");
		goto out;
	}
	dev = netdev_priv(net);

	//ether_dev = udev->usb_dev; //null error
	//ether_dev = &udev->dev; //'usb'
	//ether_dev = &xdev->dev; //'usb'
	//ether_dev = &xdev->usb_classdev; //no error NULL
	ether_dev = &net->dev; //'net' /devices/platform/s5pv210-ehci/usb1/1-1/1-1:1.0/net/eth1
	//ether_dev = smsc9500_dev; //'smsc9500'
	...
	dev->msg_enable = netif_msg_init (msg_level, NETIF_MSG_DRV
				| NETIF_MSG_PROBE | NETIF_MSG_LINK | NETIF_MSG_TIMER | NETIF_MSG_IFDOWN | NETIF_MSG_IFUP
				| NETIF_MSG_INTR | NETIF_MSG_TX_DONE | NETIF_MSG_RX_STATUS | NETIF_MSG_PKTDATA
				| NETIF_MSG_HW | NETIF_MSG_WOL | NETIF_MSG_TX_QUEUED | NETIF_MSG_RX_ERR | NETIF_MSG_TX_ERR);
#endif
	...
	if (!dev->MyWorkQueue) {
		deverr (dev,"can't create MyWorkQueue!\n");
		goto out1;
	}
	#if (LINUX_VERSION_CODE < KERNEL_VERSION(2,6,20))
...
//	add_timer(&(dev->LinkPollingTimer));
	...
	//devinfo (dev, "random address : %02x:%02x:%02x:%02x:%02x:%02x\n",
	//		net->dev_addr [0], net->dev_addr [1],
	//		net->dev_addr [2], net->dev_addr [3],
	//		net->dev_addr [4], net->dev_addr [5]);
...
// dma_supported() is deeply broken on almost all architectures
	...
	if (info->bind) {
		...
		if ((dev->driver_info->flags & FLAG_ETHER) != 0
			...
			{
#if 0	// we want static name allocation as "eth1"
	...
	if (status)
	{
		printk("[smscusbnet] register_netdev error!\n");
		goto out3;
	}
	if (netif_msg_probe (dev))
	{
...
out2:
	...
	// should send uevent before free_netdev()
	//mutex_lock(&state_lock);
	ethernet_state = 0; //notready
	if(send_uevent_smsc_state(&ether_dev->kobj, ethernet_state) < 0)
	{
		printk("[smscusbnet_probe] kobject_uevent fail, ethernet state in probe %d\n", ethernet_state);
	}
	//mutex_unlock(&state_lock);
...
EXPORT_SYMBOL_GPL(ether_dev);
EXPORT_SYMBOL_GPL(smsc9500_dev);
EXPORT_SYMBOL_GPL(ethernet_state);
//EXPORT_SYMBOL_GPL(state_lock);
EXPORT_SYMBOL_GPL(send_uevent_smsc_state);
EXPORT_SYMBOL_GPL(get_smsc_ethernet_state);
...
static int __init smscusbnet_init(void)
{
	...
	printk("smscusbnet_init\n");
	random_ether_addr(node_id);
	//wake_lock_init(&smscusbnet_wake_lock, WAKE_LOCK_SUSPEND, "smscusbnet_present");
...
{
	//printk("smscusbnet_exit\n");
	//wake_lock_destroy(&smscusbnet_wake_lock);
}
