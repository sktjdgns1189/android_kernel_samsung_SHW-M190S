...
//unsigned int debug_mode = DBG_WARNING | DBG_INIT | DBG_LINK_CHANGE | DBG_HOST |
//	DBG_MCAST | DBG_CLOSE | DBG_IOCTL | DBG_TX | DBG_RX | DBG_TRACE | DBG_LINK |
//	DBG_PWR;
//unsigned int debug_mode = DBG_WARNING | DBG_INIT | DBG_LINK_CHANGE | DBG_IOCTL | DBG_PWR;
//unsigned int debug_mode = DBG_WARNING | DBG_INIT | DBG_LINK_CHANGE;
//unsigned int debug_mode = DBG_WARNING | DBG_LINK_CHANGE;
//unsigned int debug_mode = DBG_LINK_CHANGE;
...
//u32 link_mode=LINK_AUTO_NEGOTIATE;
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,0))
...
extern int ethernet_state;
extern struct device *smsc9500_dev;
extern struct device *ether_dev;
extern int send_uevent_smsc_state(struct kobject *kobj, int data);
extern void set_ethernet_state(int state);
extern bool charging_mode_get(void);

//extern struct mutex state_lock;
...
static int _AtoN(char ch)
{
	...
	if(ch >='0' && ch <='9') {
		...
		printk("[smsc9500] _AtoN : unknown char : %c\n", ch);
		ret = 0;
	}
...
/***************************************************************/
/* read MAC address from NV file */
static int read_mac_address_from_nv(void)
{
	struct file *fpnv = NULL;
	char* nvfilepath1 = "/data/.lanmac.info";
	char* nvfilepath2 = "/data/.nvmac.info";
	char buf[18] = {0};
	int ret;

	fpnv = filp_open(nvfilepath1, O_RDONLY, 0);
	if ((fpnv == NULL) || IS_ERR(fpnv)) {
		printk("[smsc9500] nv_file1 open error! (try again)\n");
	}
	else {
		ret = kernel_read(fpnv, 0, buf, 18);
		buf[17] ='\0';   // to prevent abnormal string display when mac address is displayed on the screen.
		//printk("[smsc9500] Read MAC1 : [%s] [%d] \r\n" , buf, strncmp(buf , "000000000000" , 12));
		
		if((strncmp(buf , "0000F0973011" , 12) == 0) || (strncmp(buf , "000000000000" , 12) == 0)) {
			printk("[smsc9500] nv_file1 is default mac or all zero! (try again)\n");
	...
	if((fpnv == NULL) || IS_ERR(fpnv)) {
		fpnv = filp_open(nvfilepath2, O_RDONLY, 0);	
		if ((fpnv == NULL) || IS_ERR(fpnv)) {
			printk("[smsc9500] nv_file2 open error! (return)\n");
			return 0;
		}
		else {
			ret = kernel_read(fpnv, 0, buf, 18);
			buf[17] ='\0';   // to prevent abnormal string display when mac address is displayed on the screen.
			//printk("[smsc9500] Read MAC2 : [%s] [%d] \r\n" , buf, strncmp(buf , "000000000000" , 12));

			if(strncmp(buf , "000000000000" , 12) == 0) {
				printk("[smsc9500] mac value from nv_file are all zero!\n");
				filp_close(fpnv, NULL);
				return 0;
			}
		}
	...
	if(strncmp(&buf[12], "5",1)==0) {  // this MAC is ok.
		mac_addr_lo32 = _AtoN(buf[6]) << 28 | _AtoN(buf[7]) << 24 | _AtoN(buf[4]) << 20 | _AtoN(buf[5]) << 16 |
						_AtoN(buf[2]) << 12 | _AtoN(buf[3]) << 8 | _AtoN(buf[0]) << 4 | _AtoN(buf[1]);
		mac_addr_hi16 = _AtoN(buf[10]) << 12 | _AtoN(buf[11]) << 8 | _AtoN(buf[8]) << 4 | _AtoN(buf[9]);
		//printk("[smsc9500] lo32 = 0x%08x, hi16 = 0x%08x\n", mac_addr_lo32, mac_addr_hi16);
		
		filp_close(fpnv, NULL);
	}
	else {
		printk("[smsc9500] MAC from the file is not OK\n");
		return 0;
	}
...
/***************************************************************/
static ssize_t ethernet_state_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	int ret = 0;
	//printk("[SMSC9500]%s\n ", __func__);
...
/* for sysfs control (/sys/class/sec/switch/ethernet_state) */
static DEVICE_ATTR(ethernet_state, S_IRUGO, ethernet_state_show, NULL);

#if 0
/***************************************************************/
static ssize_t ethernet_address_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	int ret = 0;
	//printk("[SMSC9500]%s\n ", __func__);
	...
	ret = sprintf(buf, "%02x:%02x:%02x:%02x:%02x:%02x",
		    eth1_net->dev_addr [0], eth1_net->dev_addr [1],
			eth1_net->dev_addr [2], eth1_net->dev_addr [3],
			eth1_net->dev_addr [4], eth1_net->dev_addr [5]); //MAC address
#endif
			
	return ret;
}

static ssize_t ethernet_address_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
	int ret = 0;
	
	printk("[SMSC9500]%s (is not support)\n ", __func__);
...
/* for sysfs control (/sys/class/sec/switch/ethernet_address) */
static DEVICE_ATTR(ethernet_address, S_IRUGO |S_IWUGO | S_IRUSR | S_IWUSR, ethernet_address_show, ethernet_address_store);
#endif
...
static int Phy_UpdateLinkMode(struct usbnet *dev)
{
	...
	if(dwOldLinkSpeed!=(adapterData->dwLinkSpeed)) {
		if(adapterData->dwLinkSpeed!=LINK_OFF) {
			...
			//printk("[smsc9500] LINK UP >>>>>>>>>>>>>>>>>>>>>>>>>>\n\n");
			//mutex_lock(&state_lock);
			ethernet_state = 2; //plugged
			set_ethernet_state(2);
			if(send_uevent_smsc_state(&ether_dev->kobj, ethernet_state) < 0)
			{
				printk("[Phy_UpdateLinkMode] kobject_uevent fail, ethernet state : %d\n", ethernet_state);
			}
			//mutex_unlock(&state_lock);
			...
			//printk("[smsc9500] LINK DOWN >>>>>>>>>>>>>>>>>>>>>>>>>>\n\n");
			//mutex_lock(&state_lock);
			ethernet_state = 1; //unplugged
			if(send_uevent_smsc_state(&ether_dev->kobj, ethernet_state) < 0)
			{
				printk("[Phy_UpdateLinkMode] kobject_uevent fail, ethernet state : %d\n", ethernet_state);
			}
...
//USE_INTERNAL_PHY
	...
	/* set EDPD settings */
	if((dev->chipID == ID_REV_9500A_CHIPID) && (EDPDConfig != 0)) {
...
static int smsc9500_reset(struct usbnet *dev)
{
    ...
    if(!((dev->chipID == ID_REV_9500A_CHIPID) && (auto_mdix_mode == TRUE)))
		dev->chipDependFeatures[FEATURE_MDIX_MODE] = FALSE;
    /*******Enable new features**************/
	...
	if(adapterData->macAddrHi16 == 0x00000070UL || adapterData->MmacAddrLo32 == 0x110F8000UL) {
		adapterData->macAddrHi16 = 0xFFFFFFFF;
		adapterData->MmacAddrLo32 = 0xFFFFFFFF;
	}
...
#else /* use random address from smscusbnet init routine.*/
		/* read from file */
		if(read_mac_address_from_nv()) {
			dwAddrH=mac_addr_hi16;
			dwAddrL=mac_addr_lo32;
		}
		else {
			dwAddrH=0x00000070UL;
			dwAddrL=0x110F8000UL;
			//printk("[smsc9500] Mac Address is set by default to 0x%04X%08X\n",	dwAddrH,dwAddrL);
		}
#endif
	...
	//SMSC_TRACE(DBG_WARNING,"dev->net->dev_addr %02x:%02x:%02x:%02x:%02x:%02x\n",
	printk("dev->net->dev_addr %02x:%02x:%02x:%02x:%02x:%02x\n",
	...
	if (netif_running (dev->net))
	{
		...
		{//Link is down
			if(dev->netDetach){//Highest priority
				if(!dev->pmLock){//Resume immediately, then detach device from USB bus.
					//printk("[smsc9500] Smsc9500AutoSuspend, EVENT_IDLE_RESUME\n");
		...
		if(ret != SMSC9500_SUCCESS){
			...
			if(!dev->pmLock){//Resume immediately
				//printk("[smsc9500] Smsc9500AutoSuspend (2), EVENT_IDLE_RESUME\n");
				smscusbnet_defer_myevent(dev, EVENT_IDLE_RESUME);
			}
			goto _SUSPEND_EXIT;
		}else{
...
static int Smsc9500AutoResume(struct usb_interface *intf)
{
	...
	if(!dev->pmLock){
		//printk("[smsc9500] Smsc9500AutoResume, EVENT_IDLE_RESUME\n");
		smscusbnet_defer_myevent(dev, EVENT_IDLE_RESUME);
	}
...
static int smsc9500_device_recovery(struct usbnet *dev)
{
	...
	//printk("[smsc9500] smsc9500_device_recovery.............\n");
...
{
	struct class *smsc9500_class;

	if(charging_mode_get())
		return 0;

	printk("[smsc9500] smsc9500_init............\n");

	smsc9500_class = class_create(THIS_MODULE, "smsc9500");
	
	smsc9500_dev = device_create(smsc9500_class, NULL, 0, NULL, "smsc9500");

	//if (device_create_file(switch_dev, &dev_attr_ethernet_state) < 0)
	if (device_create_file(smsc9500_dev, &dev_attr_ethernet_state) < 0)
	{
		printk("Failed to create device file(%s)!\n", dev_attr_ethernet_state.attr.name);
	}

#if 0
	//if (device_create_file(switch_dev, &dev_attr_ethernet_state) < 0)
	if (device_create_file(smsc9500_dev, &dev_attr_ethernet_address) < 0)
	{
		printk("Failed to create device file(%s)!\n", dev_attr_ethernet_address.attr.name);	
	}
#endif
	
	if (IS_ERR(smsc9500_dev))
		printk("[smsc9500] Failed to create device(smsc9500)!\n");
...
{
	//printk("[smsc9500] smsc9500_exit............\n");

	device_remove_file(smsc9500_dev, &dev_attr_ethernet_state);
	//device_remove_file(switch_dev, &dev_attr_ethernet_state);

#if 0
	device_remove_file(smsc9500_dev, &dev_attr_ethernet_address);
	//device_remove_file(switch_dev, &dev_attr_ethernet_address);
#endif
