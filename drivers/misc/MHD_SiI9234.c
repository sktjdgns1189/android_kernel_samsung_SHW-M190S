...
static void EnableTMDS(void);
static void DisableTMDS(void);
...
int MHD_HW_IsOn(void)
{
	...
	return SII9234_HW_IsOn(SII9234_i2c_client);
}
...
byte MHD_Bridge_detect(void)
{
	...
	if(!gpio_get_value(GPIO_ACCESSORY_INT)&& MHD_HW_IsOn())
	{
		...
		printk("[MHD] Bridge detect %x :: HPD %d\n",BridgeOn,gpio_get_value(GPIO_MHL_HPD));
...
void I2C_WriteByte(byte deviceID, byte offset, byte value)
{
	if(deviceID == 0x72)
		...
		pr_info("[MHL]I2C_WriteByte error %x\n",deviceID); 
}
...
byte I2C_ReadByte(byte deviceID, byte offset)
{
	...
	if(deviceID == 0x72)
		...
		pr_info("[MHL]I2C_ReadByte error %x\n",deviceID);
	return (number);
}
...
void sii9234_register_init(void)
{
	...
	I2C_WriteByte(0x72, 0xA3, 0xF3);			// SIMG: Output Swing  default EB
	I2C_WriteByte(0x72, 0xA6, 0x0C);			// SIMG: Swing Offset
