...
#define YAS_BMA222_GRAVITY_EARTH                                          320  // PMD  9806550
...
#define YAS_BMA222_DEFAULT_POSITION                                             2
...
#define MAX_count_calibration 40
/* --------------------------------------------------------------------------- */
  ...
  int count_calibration =0;
...
#ifdef DEBUG
        printk(KERN_INFO "wait 50ms and got cal ready flag is %d\n",tmp);
#endif  
	if(count_calibration>=MAX_count_calibration){//MAX 2SEC(50ms X 40)
 	    	count_calibration =0;
		tmp =1;
		printk(KERN_ERR " cal_ready1 FAIL \n");
		return -1;
	}else {
		count_calibration++;
	}
   ...
   yas_bma222_get_offset_filt_x(&tmp);
	...
	if(count_calibration>=MAX_count_calibration){//MAX 2SEC(50ms X 40)
 	    	count_calibration =0;
		tmp =1;
		printk(KERN_ERR "  cal_ready2 FAIL\n");
		return -1;
	}else {
		count_calibration++;
	}
	...
	if(count_calibration>=MAX_count_calibration){//MAX 2SEC(50ms X 40)
 	    	count_calibration =0;
		tmp =1;
		printk(KERN_ERR " cal_ready3 FAIL\n");
		return -1;
	}else {
		count_calibration++;
	}
...
#ifdef DEBUG
        printk(KERN_INFO "wait 20ms and got eeprom writing status is %d\n",tmp);
#endif  
	if(count_calibration>=(MAX_count_calibration+60)){//MAX 2SEC(20ms X 100)
 	    	count_calibration =0;
		tmp =1;
		printk(KERN_ERR " eeprom_writing_status FAIL\n");
		return -1;
	}else {
		count_calibration++;
	}
