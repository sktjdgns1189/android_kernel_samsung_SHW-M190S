...
#define YAS_ACC_KERNEL_NAME                                                   "bma023"
...
static int yas_acc_core_driver_init(struct yas_acc_private_data *data)
{
	...
	if( HWREV == 3 ){
	   err = driver->set_position(0);
	}	   
	else if( HWREV == 4){
	   err = driver->set_position(3);
	}	   
	else if( HWREV == 5){
	   err = driver->set_position(3);
	}	   
	else{
	   err = driver->set_position(YAS_BMA222_DEFAULT_POSITION);
	}	   
...
{
	struct yas_acc_private_data *bma023 = container_of(file->private_data,
						struct yas_acc_private_data,
						bma023_device);
...
{
	struct yas_acc_private_data *bma023 = file->private_data;
	int try;
	int err = 0;
	...
	case BMA023_CALIBRATION:
		yas_acc_measure(bma023->driver, &accel);
		printk("BMA222_CALIBRATION data(%10d %10d %10d) raw(%5d %5d %5d)\n",
			   accel.xyz.v[0], accel.xyz.v[1], accel.xyz.v[2], accel.raw.v[0], accel.raw.v[1], accel.raw.v[2]);
		err = yas_acc_fast_calibration(bma023->driver, data_cal); 	 // calibration start
		yas_acc_measure(bma023->driver, &accel);
		printk("BMA222_CALIBRATION data(%10d %10d %10d) raw(%5d %5d %5d)\n",
			   accel.xyz.v[0], accel.xyz.v[1], accel.xyz.v[2], accel.raw.v[0], accel.raw.v[1], accel.raw.v[2]);
		break;
...
static int yas_acc_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	data->bma023_device.minor = MISC_DYNAMIC_MINOR;
	data->bma023_device.name = "accelerometer_sensor";
	data->bma023_device.fops = &bma023_fops;

	err = misc_register(&data->bma023_device);
	if (err) {
