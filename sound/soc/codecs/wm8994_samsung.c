...
select_route universal_wm8994_playback_paths[] =
{
	...
	wm8994_set_playback_hdmi_speaker_tvout,
	wm8994_set_playback_hdmi_speaker_headset_tvout
};
 ...
 */
static const char *playback_path[] = {"OFF", "RCV", "SPK", "HP", "3POLE", "BT", "SPK_HP", "EXTRA_DOCK_SPEAKER", "HDMI_TV_OUT", "HDMI_SPK", "HDMI_DUAL"};
	...
	struct snd_ctl_elem_value *ucontrol)
{
	...
	switch(path_num)
	{
		...
		case HDMI_SPK:
		case HDMI_DUAL:
			DEBUG_LOG("routing PLAYBACK HDMI PATH to %s", mc->texts[path_num]);
#if 1
			/* HDMI_USE_AUDIO - set bclk to 32fs for 44.1kHz 16 bit playback */
			if(wm8994->fs == 44100)
	...
	struct snd_ctl_elem_value *ucontrol)
{
	...
	if( (wm8994->cur_path == BT) && (path_num != BT) )
	{
		...
		wm8994_write(codec,0x303, 0x0040); // use default bclk value 0x40
