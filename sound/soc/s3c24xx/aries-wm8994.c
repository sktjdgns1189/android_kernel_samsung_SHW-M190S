	...
	struct snd_pcm_hw_params *params)
{
	...
	if (ret < 0) {
		printk(KERN_ERR "smdkc110_wm8994_hw_params : AP sys clock INT setting error!\n");
		return ret;
	}
	...
	if (ret < 0) {
		printk(KERN_ERR "smdkc110_wm8994_hw_params : AP sys clock I2SEXT setting error!\n");
		return ret;
	}
	...
	if (ret < 0) {
		printk(KERN_ERR "smdkc110_wm8994_hw_params : AP sys clock setting error!\n");
		return ret;
	}
#endif
	clk_out = clk_get(NULL, "clk_out");
	if (IS_ERR(clk_out)) {
			printk(KERN_ERR "failed to get CLK_OUT\n");
			return -EBUSY;
	}
	...
	if (clk_set_parent(clk_out, clk_epll)) {
		printk(KERN_ERR "failed to set CLK_EPLL as parent of CLK_OUT\n");
		clk_put(clk_out);
		clk_put(clk_epll);
		return -EBUSY;
	}
	...
	if (ret < 0) {
		printk(KERN_ERR "smdkc110_wm8994_hw_params : Codec sys clock setting error!\n");
		return ret;
	}
	...
	psr -= 1;
	printk(KERN_INFO "SRC_CLK=%d PSR=%d RFS=%d BFS=%d\n", SRC_CLK, psr, rfs, bfs);

	/* Set the AP Prescalar/Pll */
	ret = snd_soc_dai_set_clkdiv(cpu_dai, S3C_I2SV2_DIV_PRESCALER, psr);

	if (ret < 0) {
		printk(KERN_ERR "smdkc110_wm8994_hw_params : AP prescalar setting error!\n");
		return ret;
	}
	...
	if (ret < 0) {
		printk(KERN_ERR "smdkc110_wm8994_hw_params : AP RFS setting error!\n");
		return ret;
	}
	...
	if (ret < 0) {
		printk(KERN_ERR "smdkc110_wm8994_hw_params : AP BCLK setting error!\n");
		return ret;
	}
