...
struct max8998_charger_callbacks {
	...
	void (*set_jig)(struct max8998_charger_callbacks *ptr, int status);
	void (*lowbat_interrupt)(struct max8998_charger_callbacks *ptr);
};
...
struct max8998_charger_data {
	...
	int (*get_dock_intval)(void);
	int (*get_dock_operation)(void);
	void (*dock_earlysuspend_ctrl)(bool is_early_suspend);
	void (*dock_usbctrl)(bool enable);
};
